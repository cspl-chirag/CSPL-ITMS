...
import com.example.hp.superadminitms.Adapter.ReportInwardDataAdapter;
...
import com.example.hp.superadminitms.Network.ReportInwardResponse;
...
public class Admin_InwardReportActivity extends BaseActivity implements ConnectivityReceiver.ConnectivityReceiverListener, View.OnClickListener {
    private Button btnShow, btnClear;
    private LinearLayout llDataNotFound, llDataFound;
    private RecyclerView rvInwardReport;
    ...
    private List<StockInwardReportDatum> inwardReportData = new ArrayList<>();
    private ReportInwardDataAdapter inwardReportAdapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_admin__inward_report);
        initializeToolBar();
        initializeControls();
    }
    ...
    private void initializeToolBar() {
        ...
        getSupportActionBar().setTitle("Inward Report");
    ...
    private void initializeControls() {
        ...
        rvInwardReport = findViewById(R.id.rvInwardReport);
       ...
       switch (view.getId())
       {
           ...
           case R.id.ivStartDate:
               ...
               else
               {
                   ...
                   DatePickerDialog startDatePickerDialog = new DatePickerDialog(Admin_InwardReportActivity.this, new DatePickerDialog.OnDateSetListener() {
                       @Override
           ...
           case R.id.ivEndDate:
               ...
               else
               {
                   ...
                   mMonth = c.get(Calendar.MONTH);
                   mDay = c.get(Calendar.DAY_OF_MONTH);
                   DatePickerDialog endDatepickerDialog = new DatePickerDialog(Admin_InwardReportActivity.this, new DatePickerDialog.OnDateSetListener() {
                       @Override
                       public void onDateSet(DatePicker datePicke, int year, int monthOfYear, int dayOfMonth)
                       {
    ...
    private void getReportData() {
        ...
        pg.show();
        Call<ReportInwardResponse> call = apiInterface.getStockInwardReport(new ReportPartReplacementRequest(Company_Id, Selected_Start_Date, Selected_End_Date));
        call.enqueue(new Callback<ReportInwardResponse>() {
            @SuppressLint("LongLogTag")
            @Override
            public void onResponse(Call<ReportInwardResponse> call, Response<ReportInwardResponse> response) {
                pg.dismiss();
                try {
                    if (response.isSuccessful()) {
                        if (response.body().getStatusCode() == 1) {
                            Log.e("INWARD REPORT RESPONSE : ", response.body().getMessage());
                            inwardReportData = response.body().getStockInwardReport();
                            rvInwardReport.setLayoutManager(new GridLayoutManager(getApplicationContext(), 1));
                            inwardReportAdapter = new ReportInwardDataAdapter(Admin_InwardReportActivity.this, inwardReportData);
                            rvInwardReport.setAdapter(inwardReportAdapter);
            ...
            @SuppressLint("LongLogTag")
            @Override
            public void onFailure(Call<ReportInwardResponse> call, Throwable t) {
                pg.dismiss();
                Log.e("INWARD REPORT RESPONSE FAILURE : ", t.getMessage());
                llDataFound.setVisibility(View.GONE);
                llDataNotFound.setVisibility(View.VISIBLE);
                AlertDialog.Builder builder =
                        new AlertDialog.Builder(Admin_InwardReportActivity.this);
