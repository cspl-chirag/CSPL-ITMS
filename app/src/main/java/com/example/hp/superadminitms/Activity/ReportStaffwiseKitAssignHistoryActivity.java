...
import com.example.hp.superadminitms.Adapter.HistoryAssignKitDataAdapter;
...
import com.example.hp.superadminitms.Network.HistorytStaffAssignKitResponse;
...
public class ReportStaffwiseKitAssignHistoryActivity extends BaseActivity implements ConnectivityReceiver.ConnectivityReceiverListener, View.OnClickListener {
    ...
    private HistoryAssignKitDataAdapter adapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_report_staffwise_kit_assign_history);
        initializeToolBar();
        initializeControls();
    }
        ...
        switch (view.getId())
        {
            ...
            case R.id.ivStartDate:
                if (SystemClock.elapsedRealtime() - mLastClickTime < 1000)
                    ...
                    DatePickerDialog startDatePickerDialog = new DatePickerDialog(ReportStaffwiseKitAssignHistoryActivity.this, new DatePickerDialog.OnDateSetListener() {
                        @Override
            ...
            case R.id.ivEndDate:
                if (SystemClock.elapsedRealtime() - mLastClickTime < 1000)
                    ...
                    mMonth = c.get(Calendar.MONTH);
                    mDay = c.get(Calendar.DAY_OF_MONTH);
                    DatePickerDialog endDatepickerDialog = new DatePickerDialog(ReportStaffwiseKitAssignHistoryActivity.this, new DatePickerDialog.OnDateSetListener() {
                        @Override
                        public void onDateSet(DatePicker datePicke, int year, int monthOfYear, int dayOfMonth) {
                            try {
        ...
        if (view.getId() == R.id.btnShow) {
            ...
            if (SystemClock.elapsedRealtime() - mLastClickTime < 1000) {
                ...
                DatePickerDialog startDatePickerDialog = new DatePickerDialog(
                        ReportStaffwiseKitAssignHistoryActivity.this,
...
//        Log.e("Selected_End_Date",Selected_End_Date);
        ...
        Call<HistorytStaffAssignKitResponse> call = apiInterface.getStaffWiseKitAssignHistory(new StaffKitRequest(Company_Id, Selected_Start_Date, Selected_End_Date));
        call.enqueue(new Callback<HistorytStaffAssignKitResponse>() {
            @SuppressLint("LongLogTag")
            @Override
            public void onResponse(Call<HistorytStaffAssignKitResponse> call, Response<HistorytStaffAssignKitResponse> response) {
                try {
                    if (response.isSuccessful()) {
                        if (response.body().getStatusCode() == 1) {
                            ...
                            data = response.body().getStaffWiseKitAssignHistory();
                            adapter = new HistoryAssignKitDataAdapter(ReportStaffwiseKitAssignHistoryActivity.this, data);
            ...
            @SuppressLint("LongLogTag")
            @Override
            public void onFailure(Call<HistorytStaffAssignKitResponse> call, Throwable t) {
                pg.dismiss();
                Log.e("Staff_Assignkit_History_Failure : ", t.getMessage());
                llDataFound.setVisibility(View.GONE);
                llDataNotFound.setVisibility(View.VISIBLE);
                AlertDialog.Builder builder =
                        new AlertDialog.Builder(ReportStaffwiseKitAssignHistoryActivity.this);
