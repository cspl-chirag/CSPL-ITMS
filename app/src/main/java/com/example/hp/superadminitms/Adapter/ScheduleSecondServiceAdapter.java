...
public class ScheduleSecondServiceAdapter extends RecyclerView.Adapter<ScheduleSecondServiceAdapter.ViewHolder> {
    private final Context context;
    private final List<SecondScheduleService> scheduleServices;

    public ScheduleSecondServiceAdapter(Context context, List<SecondScheduleService> scheduleServices) {
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public void setData(SecondScheduleService SecondScheduleService, int position) {
            tvSrNo.setText(String.valueOf(position + 1));
            tvVehicleRegNo.setText(SecondScheduleService.getVehicleName());
            try {
                tvLogDate.setText(Common.convertDateFormat(SecondScheduleService.getLogDate(), "yyyy-MM-dd'T'hh:mm:ss", "dd-MM-yyyy"));
            } catch (ParseException e) {
                e.printStackTrace();
            }
            tvFirstServiceKM.setText(String.valueOf(SecondScheduleService.getSecondServiceKm()));
        }
    }
}
