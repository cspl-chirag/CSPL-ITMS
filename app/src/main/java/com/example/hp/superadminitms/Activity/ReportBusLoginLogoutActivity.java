...
import com.example.hp.superadminitms.Network.BusLoginLogoutReportResponse;
...
public class ReportBusLoginLogoutActivity extends BaseActivity implements ConnectivityReceiver.ConnectivityReceiverListener, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_report_bus_login_logout);
        initializeToolBar();
        initializeControls();
    }
    ...
    private void initializeToolBar() {
        ...
        getSupportActionBar().setTitle("Buswise Login Logout Report");
    ...
    private void getReportData() {
        ...
        if (!isConnected) {
            ...
            pg.setMessage("Getting BusLoginLogout Data...");
            pg.show();
            ApiInterface apiInterface = ApiClient.getClient().create(ApiInterface.class);
            Log.e("SELECTED DATE : ", Selected_Date);
            Call<BusLoginLogoutReportResponse> call = apiInterface.getBusLoginLogoutReport(new ReportPartReplacementRequest(Company_Id, Selected_Date));
            call.enqueue(new Callback<BusLoginLogoutReportResponse>() {
                public Integer total = 0;

                @SuppressLint("LongLogTag")
                @Override
                public void onResponse(Call<BusLoginLogoutReportResponse> call, Response<BusLoginLogoutReportResponse> response) {
                    pg.dismiss();
                    try {
                        if (response.isSuccessful()) {
                            if (response.body().getStatusCode() == 1) {
                                ...
                                reportData = response.body().getBusLogInLogOutReportData();
                                for (int i = 0; i < reportData.size(); i++) {
                                    total += reportData.get(i).getRunKm();
                                }
                                tvTotal.setText(String.valueOf(total));
                                adapter = new ReportBusLoginLogoutAdapter(ReportBusLoginLogoutActivity.this, reportData);
                ...
                @SuppressLint("LongLogTag")
                @Override
                public void onFailure(Call<BusLoginLogoutReportResponse> call, Throwable t) {
                    pg.dismiss();
                    llDataFound.setVisibility(View.GONE);
                    llDataNotFound.setVisibility(View.VISIBLE);
        ...
        switch (view.getId()) {
            ...
            case R.id.ivStartDate:
                if (SystemClock.elapsedRealtime() - mLastClickTime < 1000)
                    ...
                    DatePickerDialog startDatePickerDialog = new DatePickerDialog(ReportBusLoginLogoutActivity.this, new DatePickerDialog.OnDateSetListener() {
                        @Override
        ...
        if (view.getId() == R.id.btnShow) {
            ...
            if (SystemClock.elapsedRealtime() - mLastClickTime < 1000) {
                ...
                DatePickerDialog startDatePickerDialog = new DatePickerDialog(
                        ReportBusLoginLogoutActivity.this,
