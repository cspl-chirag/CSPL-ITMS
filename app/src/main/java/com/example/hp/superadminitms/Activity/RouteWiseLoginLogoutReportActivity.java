...
import com.example.hp.superadminitms.Adapter.RouteWiseBusLoginLogoutAdapter;
...
import com.example.hp.superadminitms.Network.RoutewiseBusLoginLogoutResponse;
...
public class RouteWiseLoginLogoutReportActivity extends BaseActivity implements ConnectivityReceiver.ConnectivityReceiverListener, View.OnClickListener {
    ...
    private List<RouteWiseBusLogInLogOutReport> reportData = new ArrayList<>();
    ...
    private RouteWiseBusLoginLogoutAdapter reportDataAdapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_route_wise_login_logout_report);
        initializeToolBar();
        initializeControls();
    }
    ...
    private void initializeToolBar() {
        ...
        getSupportActionBar().setTitle("Routewise Bus Login Logout Information");
    ...
    private void getReportData() {
        ...
        if (!isConnected) {
            ...
            pg.setMessage("Getting Routewise Bus Login Logout Information...");
            pg.show();
            ApiInterface apiInterface = ApiClient.getClient().create(ApiInterface.class);
            Log.e("SELECTED DATE : ", Selected_Date);
            Call<RoutewiseBusLoginLogoutResponse> call = apiInterface.getRoutewiseBusLoginLogoutReport(new ReportPartReplacementRequest(Company_Id, Selected_Date));
            call.enqueue(new Callback<RoutewiseBusLoginLogoutResponse>() {
                @SuppressLint("LongLogTag")
                @Override
                public void onResponse(Call<RoutewiseBusLoginLogoutResponse> call, Response<RoutewiseBusLoginLogoutResponse> response) {
                    pg.dismiss();
                    try {
                        if (response.isSuccessful()) {
                            if (response.body().getStatusCode() == 1) {
                                Log.e("BUS LOGIN_LOGOUT REPORT RESPONSE : ", response.body().getMessage());
                                reportData = response.body().getRouteWiseBusLogInLogOutReport();
                ...
                @SuppressLint("LongLogTag")
                @Override
                public void onFailure(Call<RoutewiseBusLoginLogoutResponse> call, Throwable t) {
                    pg.dismiss();
                    llDataFound.setVisibility(View.GONE);
                    llDataNotFound.setVisibility(View.VISIBLE);
    ...
    private void setRecyclerviewReport(List<RouteWiseBusLogInLogOutReport> reportData) {
        if (!reportData.isEmpty()) {
            rvReport.setLayoutManager(new GridLayoutManager(getApplicationContext(), 1));
            if (Build.VERSION.SDK_INT >= 24) {
                routeNumber = reportData.stream().map(RouteWiseBusLogInLogOutReport::getRouteNo).distinct().collect(Collectors.toList());
                llDataFound.setVisibility(View.VISIBLE);
                llDataNotFound.setVisibility(View.GONE);
                reportDataAdapter = new RouteWiseBusLoginLogoutAdapter(RouteWiseLoginLogoutReportActivity.this, routeNumber, reportData);
                rvReport.setLayoutManager(new GridLayoutManager(getApplicationContext(), 1));
...
//                for (RouteWiseBusReport detailReportHcfListDatum : )
                for (RouteWiseBusLogInLogOutReport datum : reportData) {
        ...
        switch (view.getId()) {
            ...
            case R.id.ivStartDate:
                if (SystemClock.elapsedRealtime() - mLastClickTime < 1000)
                    ...
                    DatePickerDialog startDatePickerDialog = new DatePickerDialog(RouteWiseLoginLogoutReportActivity.this, new DatePickerDialog.OnDateSetListener() {
                        @Override
